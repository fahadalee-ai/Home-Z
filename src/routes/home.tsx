...
import { Bell, Heart, Search, Play, Users, ChevronRight, ShieldCheck, Flame, Clock } from "lucide-react";
    ...
    <div className="mx-auto min-h-screen max-w-[440px] pb-28">
      ...
      <div className="px-5 pt-5">
        <div className="flex items-center gap-3 rounded-xl border border-border bg-surface px-4 py-3">
          ...
          <input
            placeholder="Search auctions and live tours"
      ...
      <section className="pt-6">
        <div className="px-5">
          <SectionHeader title="Categories" cta="See all" to="/auctions" />
        </div>
        <div className="hide-scrollbar scroll-fade-x mt-3 flex gap-3 overflow-x-auto px-5 pb-1">
          {CATEGORIES.map((c) => (
            <Link to="/auctions" key={c.name} className="group w-20 shrink-0">
      ...
      <section className="px-5 pt-8">
        <div className="relative overflow-hidden rounded-2xl border border-primary/30 bg-gradient-to-br from-primary/15 via-surface to-background p-6">
          ...
          <p className="text-[10px] uppercase tracking-[0.3em] text-primary">List on Home Z</p>
          <h3 className="mt-2 font-display text-2xl font-bold leading-tight">
            Ready to <span className="text-gradient-gold">sell your home?</span>
          </h3>
          <p className="mt-2 text-sm text-muted-foreground">Submit your property to our listing specialists.</p>
          <button className="mt-4 inline-flex items-center gap-2 rounded-full bg-gradient-gold px-5 py-2.5 text-xs font-semibold uppercase tracking-widest text-primary-foreground">
            List Property <ChevronRight className="h-3.5 w-3.5" />
          </button>
        </div>
      </section>
    ...
    <Link to="/live" search={{ event: "legends-night" }} className="mt-3 block">
      <div className="overflow-hidden rounded-2xl border border-border bg-surface">
        ...
        <div className="p-3.5">
          ...
          <h3 className="mt-1 line-clamp-1 font-display text-base font-bold">Coastal Classics: Oceanfront Drop</h3>
